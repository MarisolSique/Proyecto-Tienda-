...
public class UsuariosEdicion extends JFrame {
    ...
    private void jbInit() throws Exception {
        ...
        //Obtener y popular información
            posicion = Usuarios.posicion;
            m_usuario u = new m_usuario();
            usuario = u.obtenerUsuario(posicion);
            id = usuario.id;
        jTextField1.setText(usuario.nombre);
        jTextField2.setText(usuario.correo);
        if(usuario.activo){ jRadioButton1.setSelected(true); }else{ jRadioButton2.setSelected(true); }
        for(Object sucursal : m_sucursal.getNombresSucursales()){
            jComboBox1.addItem(sucursal);
        }

        jComboBox1.setSelectedItem(m_sucursal.obtenerSucursalPorID(usuario.id_sucursal).nombre);

        jPasswordField1.setBounds(new Rectangle(175, 115, 200, 20));
        jPasswordField1.setSize(new Dimension(200, 20));
    ...
    private void jButton1_actionPerformed(ActionEvent e) {
        ...
        //Validar
        if( validar.camposVacios(jTextField1,jTextField2)  & validar.correo(jTextField2) ){
            //Si contraseña viene vacía, no actualizar
            if(contrasena.equals("") || contrasena == null){
                contrasena = usuario.contrasena;
            }
            //Si valida, actualizar
            m_usuario usr = new m_usuario();
            if( usr.editarUsuario(posicion, id, nombre, correo, contrasena, activo, id_sucursal) ){
